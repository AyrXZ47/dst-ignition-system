...
| 4 | Release: Gerbers, BOM, PDF final, security-audit | [x] gerbers+BOM exportados 2026-09-16 (`hardware/fab/ignition-system-jlcpcb.zip`) — falta: pedido JLCPCB + security-audit de release |
...
| 2026-09-16 | **Cierre del periodo wave 1-4**: fab package en `hardware/fab/` (ignition-system-jlcpcb.zip + BOM.csv), waves 1-3 audited, wave 4 exportada. Sesión planificada cerrada con todo pusheado a origin/main (698f603). Pendientes EXTERNOS al repo: pedido JLCPCB, ensamblaje, firmware (src/), security-audit operativo antes del primer disparo | El worktree/rama wave3-executor-4 (tip 709d7ee, ya en main) se elimina al cierre; el humano aprenderá a regenerar el paquete fab él mismo (logo en silkscreen → re-export) |
| 2026-09-16 | **Wave 4 ejecutada para pedido JLCPCB** (humano: "sin burocracia"). Export headless: Gerbers 9 capas (GTL/GBL/GTO/GBO/GTS/GBS/GTP/GM1) + Excellon .drl + job file → `hardware/fab/ignition-system-jlcpcb.zip` (10 files, 117.6K). BOM.csv desde `kicad-cli sch export bom` (28 refs, 27 pieces). Certificación previa al export: ERC 0/0, DRC 0 errores / 0 unconnected pads / 0 drill violations (53 warnings no bloqueantes). Placa 70×50mm ≤100×100 = tier mínimo JLCPCB | Gerbers regenerables con `kicad-cli pcb export gerbers` en cualplesquier momento; única acción restante del proyecto hardware: llenar el formulario de JLCPCB + ensamblar + security-audit operativo antes del primer disparo |
